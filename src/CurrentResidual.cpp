...
void CurrentResidual::init(mc_control::MCGlobalController & controller, const mc_rtc::Configuration & config)
{
  ...
  if(kt.empty())
  {
    kt = {{"joint_1", 0.11},  {"joint_2", 0.11},  {"joint_3", 0.11}, {"joint_4", 0.11},
          {"joint_5", 0.076}, {"joint_6", 0.076}, {"joint_7", 0.076}};
  }
  ...
  else
  {
    ...
    mc_rtc::log::warning("[CurrentResidual] No torque_fric in datastore");
  }
  ...
  for(auto const & [key, val] : kt)
  {
    ...
    mc_rtc::log::info("[CurrentResidual] Joint {} has kt value {}, motor torque current based {} N.m", key, val,
                      tau_mot);
  }
...
void CurrentResidual::addPlot(mc_control::MCGlobalController & controller)
{
  ...
  gui.addPlot("CurrentResidual", mc_rtc::gui::plot::X("t", [this]() { return counter_; }),
              mc_rtc::gui::plot::Y(
